...
/// runs a multi-iteration loop, so a read-only lookup in front of a mutation is a legitimate first
/// step there; a single-call replay would book it as a selection miss. Navigation and UI passthrough
/// skills are excluded: they end or redirect the turn in production and remain real wrong choices.
/// </summary>
...
public static class ReplayFollowUpPolicy
{
    ...
    public static bool ShouldFollowUp(AgentSkill? chosen, AgentSkill? expected) =>
        chosen != null
        && expected != null
        && chosen.Effect == SkillEffect.Read
