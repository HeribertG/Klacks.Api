...
/// <summary>
/// Iterates the enabled LLM models and runs the Wizard 3 pre-flight ping against each one.
/// Returns a per-model health summary so the operator can pick a model that is reachable,
/// fast and JSON-format-compliant without paying for a full Wizard 3 run.
/// </summary>
public sealed class Wizard3ModelCheckService
{
    private readonly ILLMRepository _llmRepository;
    private readonly IPlanProposalProvider _proposalProvider;
    private readonly ILogger<Wizard3ModelCheckService> _logger;

    public Wizard3ModelCheckService(
        ILLMRepository llmRepository,
        IPlanProposalProvider proposalProvider,
        ILogger<Wizard3ModelCheckService> logger)
    {
    ...
    public async Task<IReadOnlyList<Wizard3ModelCheckResult>> CheckAllAsync(CancellationToken cancellationToken)
    {
        ...
        var results = new List<Wizard3ModelCheckResult>(models.Count);
        foreach (var model in models)
        {
            ...
            try
            {
                ...
                results.Add(new Wizard3ModelCheckResult(
...
public sealed record Wizard3ModelCheckResult(
    string ModelId,
    string DisplayName,
    string ProviderId,
    bool IsHealthy,
