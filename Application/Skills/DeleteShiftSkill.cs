...
/// <summary>
/// Soft-deletes a shift. Refuses when the shift has cuts (a nested-set sub-shift tree) or when it
/// still has works: get_schedule_entries joins the shift table without a soft-delete filter, so a
/// deleted shift with works would keep rendering against a dead definition. Both cases are reported
/// as a manual-action message. Dispatches DeleteCommand&lt;ShiftResource&gt;.
/// </summary>
...
public class DeleteShiftSkill : BaseSkillImplementation
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            return SkillResult.Error("This shift has cuts (a sub-shift tree); deleting a cut parent is not yet supported — resolve the cuts manually first.");
        }
