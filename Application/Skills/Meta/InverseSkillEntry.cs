...
/// invisible to SkillRiskClassifier. Reversibility there is not a statement about the data, it is the
/// autonomy gate: a Reversible skill runs unattended where an Irreversible one is held for confirmation.
/// The entries added 2026-09-16 do declare a lossless inverse, but letting that silently release seven
/// write skills from the gate is a separate decision and needs the owner's approval (spec §8), so they
/// are marked here and the risk class of every seeded skill stays exactly as SkillRiskReversibilityPinTests
/// pinned it. Lifting the flag for an approved pair is a one-word diff, and the pin test will show the
/// autonomy change it causes.
/// </param>
...
public sealed record InverseSkillEntry(
    ...
    bool UndoOnly = false)
{
    public IReadOnlyList<string> CopiedArguments => CopiedArgumentNames ?? [];
}
