...
/// <param name="clientId">Optional. UUID of the employee whose open clarification is closed.</param>
...
public class CloseClarificationSkill : BaseSkillImplementation
{
    ...
    private const string VerificationDescriptionFormat = "follow-up question {0}";
    ...
    public CloseClarificationSkill(IInboundClarificationRepository clarificationRepository, TimeProvider timeProvider)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await ConfirmPersistedAsync<InboundClarification>(
            SkillName,
            () => _clarificationRepository.GetByIdAsync(clarification.Id, cancellationToken),
            reread => reread.Status == InboundClarificationStatus.TakenOver,
            string.Format(CultureInfo.InvariantCulture, VerificationDescriptionFormat, clarification.Id));
        ...
        return SkillResult.SuccessResult(
            ...
            string.Format(CultureInfo.InvariantCulture, SuccessMessageFormat, clarification.SenderDisplay));
    }
