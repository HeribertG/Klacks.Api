...
/// The write goes to POST api/backend/Annotations, the endpoint that owns exactly this one row, rather
/// than reading the whole client and writing it back — that would open a lost-update window across the
/// round trip and run the full client-update machinery for a single insert. The response of the POST is
/// the confirmation that it persisted.
/// </summary>
...
public class AddClientNoteSkill : BaseSkillImplementation
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await _selfApi.PostAsync<AnnotationResource>(
            _routes.Resolve(typeof(AnnotationResource)), resource, context, SkillName, cancellationToken);
        ...
        if (result.Value == null)
        {
            return SkillResult.Error(
                $"Adding the note for client '{client.FirstName} {client.Name}' returned no result — " +
