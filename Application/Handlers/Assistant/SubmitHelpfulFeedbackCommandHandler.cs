...
/// correction endpoint does - by turn id when the client sends one, otherwise the most recent trajectory of
/// this user under the MessageNormalizer hash - so praise and complaint can never disagree about which turn
/// was meant.
        ...
        SubmitHelpfulFeedbackCommand request, CancellationToken cancellationToken)
    {
        ...
        if (trajectory == null)
        {
            _logger.LogInformation(
                "Helpful feedback found no trajectory for turn {TurnId} / hash {Hash}; the turn was not captured",
                request.TurnId, MessageNormalizer.Hash(request.UserMessage));
