...
public class VatsController : BaseController
{
    ...
    {
        _logger.LogInformation($"Deleting VAT with ID: {id}");
        var vat = await mediator.Send(new Application.Commands.Settings.Vats.DeleteCommand(id));
        if (vat == null)
        {
            _logger.LogWarning($"VAT with ID: {id} not found");
            return NotFound();
        }
        _logger.LogInformation($"VAT with ID: {id} deleted successfully");
        return vat;
    }
    ...
    public async Task<IEnumerable<Vat>> GetVats()
    {
        ...
        _logger.LogInformation($"Retrieved {result.Count()} VATs");
        return result;
    }
    ...
    {
        _logger.LogInformation($"Fetching VAT with ID: {id}");
        ...
        _logger.LogInformation($"VAT with ID: {id} retrieved successfully");
        return vat;
    }
    ...
    {
        _logger.LogInformation("Updating VAT");
