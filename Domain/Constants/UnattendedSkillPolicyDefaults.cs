...
public static class UnattendedSkillPolicyDefaults
{
    ...
    /// <summary>
    /// Threshold for an irreversible skill on the inbound-automation path (email and messenger), which
    /// has no per-task opt-in. Set to the highest level because every irreversible action the inbound
    /// intent mapping can trigger already demands FullyAutonomous there — the policy re-states that
    /// requirement instead of trusting the mapping to keep it.
    /// </summary>
    public const AutonomyLevel MinimumLevelForIrreversibleInboundAutomation = AutonomyLevel.FullyAutonomous;
}
