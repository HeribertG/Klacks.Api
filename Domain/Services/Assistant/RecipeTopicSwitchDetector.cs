...
/// A message counts as a topic switch when (a) some sentence in it opens with an interrogative lead in one
/// of the languages MutationIntentDetector.IsInformationQuestion recognizes (core languages plus any
/// plugin-configured question leads) and (b) the message carries more than one word. The word-count floor
/// exists because our own recipes hand out single-word chip replies ("show", "list", "none", ...) that
/// collide with English question leads read as bare imperatives; a real independent question is never one
/// word. Splitting into sentences (not just checking the message's own opening word) is what catches the
/// live repro: "Ich habe eine xml Datei ... Wie kann ich es einbinden?" opens with a plain statement and
/// only its second sentence is the actual question.
/// </summary>
