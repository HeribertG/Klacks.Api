...
/// channels; a retry of the very message that raised the question is ignored, and an auto-generated mail
/// such as an out-of-office notice never counts as an answer), re-analyses original
...
/// against the expiry sweep, a planner takeover or a parallel answer (the row is re-read: only Expired says
/// "expired unanswered", the others get a neutral "already closed" note), or finding a recently expired
...
public sealed class ClarificationCoordinator : IClarificationCoordinator
{
    ...
    {
        if (request.ClientType == EntityTypeEnum.Customer || request.EmailThread?.IsAutoGenerated == true)
        {
        ...
        ClarificationRequest request, InboundClarification open, CancellationToken cancellationToken)
    {
        ...
        if (!resolved)
        {
            _logger.LogInformation(
                "Clarification {ClarificationId} was resolved by someone else before the answer {SourceId} was matched; analyzing the answer normally",
                open.Id, request.Source.SourceId);
            return await DescribeLostRaceAsync(open, askedLocal, cancellationToken);
        }
    ...
    private async Task<ClarificationPreAnalysis> DescribeLostRaceAsync(
        InboundClarification open, DateTime askedLocal, CancellationToken cancellationToken)
    {
        var current = await _clarificationRepository.GetByIdAsync(open.Id, cancellationToken);
