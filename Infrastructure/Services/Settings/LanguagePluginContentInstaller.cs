...
public class LanguagePluginContentInstaller
{
    ...
    /// InstallSkillSynonymsAsync and deliberately simpler: a synonym list is merged phrase by phrase
    /// against the skill_phrase table because a user may have added phrases of their own, while a label
    /// is ONE authored string per language that nobody else writes, so the pack simply owns its key.
    /// Nothing is mirrored into skill_phrase: synonyms are input vocabulary that decides whether a skill
    /// is selected, labels are output text that names it back to the user, and a label in the matching
    /// path would change routing.
    ...
    /// cleans up after itself. That is the newer precedent set by UninstallRecipeVetoesAsync; the
    /// file-driven skill- and recipe-synonym uninstallers predate it and are not copied here.
    /// </summary>
    /// <param name="scope">Scope providing the skill repository</param>
