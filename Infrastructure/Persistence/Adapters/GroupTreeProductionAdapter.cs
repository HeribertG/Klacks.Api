...
public class GroupTreeProductionAdapter : IGroupTreeDatabaseAdapter
{
    ...
    {
        await _context.Database.ExecuteSqlRawAsync(
            "UPDATE \"group\" SET \"rgt\" = \"rgt\" + @p0 WHERE \"rgt\" > @p1 AND \"rgt\" < @p2 AND \"root\" = @p3 AND \"is_deleted\" = false",
    ...
    {
        await _context.Database.ExecuteSqlRawAsync(
            "UPDATE \"group\" SET \"lft\" = \"lft\" + @p0 WHERE \"lft\" > @p1 AND \"lft\" < @p2 AND \"root\" = @p3 AND \"is_deleted\" = false",
    ...
    {
        await _context.Database.ExecuteSqlRawAsync(
            "UPDATE \"group\" SET \"is_deleted\" = true, \"deleted_time\" = @p0 WHERE \"lft\" >= @p1 AND \"rgt\" <= @p2 AND \"root\" = @p3",
    ...
    {
        await _context.Database.ExecuteSqlRawAsync(
            "UPDATE \"group\" SET \"lft\" = -\"lft\", \"rgt\" = -\"rgt\" WHERE \"lft\" >= @p0 AND \"rgt\" <= @p1 AND \"root\" = @p2",
            lft, rgt, root);
    }
    ...
    {
        await _context.Database.ExecuteSqlRawAsync(
            "UPDATE \"group\" SET \"lft\" = -\"lft\" + @p0, \"rgt\" = -\"rgt\" + @p0, \"root\" = @p1 WHERE \"lft\" <= 0 AND \"root\" = @p2",
