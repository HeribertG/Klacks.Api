...
public class ImapEmailService : IImapEmailService
{
    ...
    private const char ThreadReferenceSeparator = ' ';
    ...
    internal static ReceivedEmail MapToReceivedEmail(MimeMessage message, UniqueId uid, string folder, string messageId)
    {
        ...
        return new ReceivedEmail
        {
            ...
            InReplyTo = string.IsNullOrWhiteSpace(message.InReplyTo) ? null : message.InReplyTo.Trim(),
            ThreadReferences = message.References.Count > 0 ? string.Join(ThreadReferenceSeparator, message.References) : null,
            IsAutoGenerated = EmailAutoGeneratedDetector.IsAutoGenerated(message.Headers)
        };
    }
